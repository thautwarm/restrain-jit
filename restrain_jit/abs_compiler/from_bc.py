...
def label_to_name(label: bc.BasicBlock):
...
class Interpreter:
    """
    From Python bytecode to finally tagless representations.
    Given such a finally tagless `result`, and a specified
    Virtual Machine `VMx` we have
        `instance_representation = run_machine(result, VMx)`
    where the `instance_representation` can be any
    - intermediate representation like LLVM IR
    - executable instructions like JVM bytecode, WASM, MIPS asm or Python bytecode
    - codes of high level language like Python, C/C++
    """

    def __init__(self, lineno_start: int):
        self.lineno = lineno_start

    def abs_i_cfg(self, b: bc.ControlFlowGraph):
        for each in b:
    ...
    def abs_i(self, b: t.Union[bc.Instr, bc.SetLineno]):
        ...
        if b.name == InstrNames.COMPARE_OP:
            ...
            raise NotImplementedError(
                f"instruction {b} not supported yet")
